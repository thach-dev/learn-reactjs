...
export default function ToDoList() {
    ...
    */
    ...
    const [listed, setListed] = useState(() => {
        const jsonListed = localStorage.getItem('listed');
        if (jsonListed) {
            return JSON.parse(jsonListed);
        }
        return [];
    });
    const handleAdd = () => {
        ...
        setListed(pre => {
            const newListed = [...pre, { id: new Date().getTime(), value: newValue }];
            const jsonListed = JSON.stringify(newListed);
            localStorage.setItem('listed', jsonListed);
            return newListed;
        })
    ...
    const handleDelete = (id) => {
        ...
        const jsonListed = JSON.stringify(listed.filter(item => item.id !== id));
        localStorage.setItem('listed', jsonListed);
    }
    return (
        <>
